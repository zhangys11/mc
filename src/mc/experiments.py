...
def pi_illustrate(N=2000):
    ...
    plt.scatter(xs, ys, s=round(N/50 + 0.5), marker='o',
...
def parcel(N=100000, num_players=5, num_ops=10):
    ...
    for _ in range(N):
        ...
        for _ in range(num_ops):
            ...
            # %表示mod运算
            position = (
                position + random.choice([-1, +1]) + num_players) % num_players
...
def dices(N=10000):
    ...
    # range: [low, high)
    samples = np.random.randint(low=1, high=7, size=(N, 3))
    dict_cnt = {}
    dict_cnt['ooo'] = 0  # 三个相同
    dict_cnt['123'] = 0
    dict_cnt['456'] = 0
    ...
    assert(dict_cnt['ooo'] + dict_cnt['xyz'] +
           dict_cnt['123'] + dict_cnt['456'] + dict_cnt['oox'] == N)
